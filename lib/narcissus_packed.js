...
Narcissus.lexer = (function() {
    ...
    Tokenizer.prototype = {
        ...
        // toka: returns list of scipped comments
        skip: function () {
                ...
                if (ch === '\n' && !this.scanNewlines) {
                    ...
                    for (;;) {
                        ...
                        if (ch === '*') {
                            ...
                            if (next === '/') {
                                this.cursor++;
                                comments.push( {type:"BLOCK", text: input.substring(cstart+1,this.cursor-2)} )
                                break;
                            }
                        } else if (ch === '\n') {
                            this.lineno++;
                    ...
                    for (;;) {
                        ch = input[this.cursor++];
                        if (ch === undefined) {
                            comments.push( {type:"LINE", text: input.substring(cstart+1,this.cursor-1) } )
                            return comments;
                        }
                        if (ch === '\n') {
                            comments.push( {type:"LINE", text: input.substring(cstart+1, this.cursor-1) } )
